...
openssl x509 -req -in UserReq.pem -days 60 -CA CACert.pem -CAkey CAPrivateKey.pem -CAcreateserial -out UserCert.pem -extfile user-ext.cnf
...
# 3. Use CA's private key to sign web server's CSR and get back the signed certificate
openssl x509 -req -in DatabaseBackupServerReq.pem -days 60 -CA CACert.pem -CAkey CAPrivateKey.pem -CAcreateserial -out DatabaseBackupServerCert.pem -extfile databaseServer-ext.cnf
######################################
